...
interface IShapeBlockProps {
  ...
  onChangeColor: (
    ...
    id: number,
    property: keyof IContentBlock | keyof ShapeBlockType,
...
const ShapeBlockComponent: React.FC<IShapeBlockProps> = ({
  ...
  return (
    <Rnd
      ...
      onDrag={(_, d) => onDrag?.(d.x, d.y, block.width, block.height)}
      onDragStop={(_, d) => {
    ...
    >
      <OutsideClickWrapper cb={() => block.isEditMode && onToggleBlockEditMode(block.id, false, "shapeBlocks")}>
        <div
          ref={shapeRef}
          className={cn(
        ...
        >
          ...
          {block.shapeType === "triangle" && (
            ...
            >
              <polygon
                points={`0,${block.height} ${block.width / 2},0 ${block.width},${block.height}`}
